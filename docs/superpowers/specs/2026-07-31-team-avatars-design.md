...
- 展开态照片形式：**方形圆角**，尺寸定为 96×96、圆角 10px
...
| 选择器 | 现值 | 改后 |
|--------|------|------|
| `.photo-img` | 50×50 圆形（`border-radius: 50%`） | **96×96、`border-radius: 10px`**、`object-fit: cover` 不变 |
| `.card-photo` 展开 max-width | `56px` | **`110px`**（容纳照片 + 左边距） |
| `.card-photo` 展开 margin-left | `14px` | 保持 `14px` |
| `.team-card:hover` height | `120px` | **`140px`**（为 96px 照片留呼吸感） |
| 无照片占位 `.avatar-circle--photo` | 50×50 圆形渐变 | **96×96 方形圆角渐变**（`border-radius: 10px`），`initials--photo` 字号改为 `1.3rem` |
...
2. 悬停展开后，中部显示 96×96 圆角真人照片，清晰不变形
